...
OUTPUT_ARGS=(
    --output_dir "/data/42-julia-hpc-rz-cv/sig95vg/DOVE/checkpoint/DOVE-s2"
    --report_to "wandb"
)
...
# Resolve script directory to reference config file
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Combine all arguments and launch training
accelerate launch --config_file "${SCRIPT_DIR}/accelerate_config.yaml" train.py \
    "${MODEL_ARGS[@]}" \
    "${LORA_ARGS[@]}" \
    "${OUTPUT_ARGS[@]}" \
    "${DATA_ARGS[@]}" \
