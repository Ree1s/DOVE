...
def compute_psg_temporal_importance(
    ...
    if N_img % num_frames != 0:
        # Fallback: uniform importance if we cannot cleanly factor into frames.
        return image_tokens.new_ones(B, N_img, device=device)

    tokens_per_frame = N_img // num_frames
    x = image_tokens.reshape(B, num_frames, tokens_per_frame, C)  # [B, F, P, C]
    ...
    if num_frames == 2:
        v_mag = v.norm(dim=-1, keepdim=False)  # [B, 1, P]
        importance = v_mag.expand(B, num_frames, tokens_per_frame)  # [B, 2, P]
        if normalize:
